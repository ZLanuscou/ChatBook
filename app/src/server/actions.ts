import { type User, type Task, type File, type Document, type Chat } from 'wasp/entities';
...
export const createChat: CreateChat<Chat> = async (chat, context) => {
  ...
  const newDocument = await context.entities.Chat.create({
    data: {
      user: { connect: { id: context.user.id } },
      document: {
...
export const createDocument: CreateDocument<docArgs, Document> = async ({ fileType, title }, context) => {
